...
def extract_feature(raw_data, selector, window=30, with_label=True, flatten=True):
    ...
    closes = numpy.array(raw_data[:,4],dtype=numpy.float)
    opens = numpy.array(raw_data[:,1],dtype=numpy.float)
    highs = numpy.array(raw_data[:,2],dtype=numpy.float)
    lows = numpy.array(raw_data[:,3],dtype=numpy.float)
    volumes = numpy.array(raw_data[:,5],dtype=numpy.float)
    if with_label:
