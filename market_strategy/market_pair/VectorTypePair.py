...
class VectorTypePair(BasePair):
    def __init__(self, coin_type, time_type,binance,stop_loss=-0.02):
        ...
        self.config["db_name"]="market_strategy_vector_type"
    ...
    def get_opportunity_vector_type(self,data):
        ...
        buy_flag=fenxing_type[-1]>fenxing_type[-2]
        sell_flag=fenxing_type[-1]<fenxing_type[-2]
        hashMap={}
        average_line={}
        self.json_save["fenxing_plot"]=fenxing_plot
        self.json_save["fenxing_type"]=fenxing_type
        if(buy_flag):
            self.market_broker(buy_flag,sell_flag, config.min_tx_volume, self.coin_type,hashMap,average_line)
        elif(sell_flag):
            self.market_broker(buy_flag,sell_flag, config.min_tx_volume, self.coin_type,hashMap,average_line)

    def get_opportunity_liner_vector_type(self,data):
        X = np.array(data)
        k_data,time_list = self.get_Fnk(X)
        ## 判断包含关系
        after_fenxing=self.get_relation(k_data)
        fenxing_data,fenxing_plot,fenxing_type=self.find_top_bottom(after_fenxing,time_list)
        print(fenxing_type)
        predictions=self.linear_model_main(fenxing_plot)
        buy_flag=fenxing_plot[-1]>fenxing_plot[-2] and fenxing_plot
        sell_flag=fenxing_type[-1]<fenxing_type[-2]
        hashMap={}
        average_line={}
        self.json_save["fenxing_plot"]=fenxing_plot
    ...
    def get_Fnk(self,X,n=1):
        data={}
        data["open"]=np.array(X[:, 1], dtype=np.float)
        data["close"]=np.array(X[:, 4], dtype=np.float)
        data["high"]=np.array(X[:, 2], dtype=np.float)
        data["low"]=np.array(X[:, 3], dtype=np.float)
        ...
        for i in range(n,length,n):
            ...
            temp_open = temp.open.values[-1]
            temp_high = max(temp.close)
            temp_low = min(temp.open)
            temp_k = temp[-1:]
        ...
        temp_high = max(last_one.close)
        temp_low = min(last_one.open)
        temp_k = last_one[-1:]
        temp_k.open = temp_open
        temp_k.high = temp_high
        temp_k.low = temp_low
    ...
    def linear_model_main(self,fenxing_plot):
        # Create linear regression object
        axis=self.get_axis_high_line(fenxing_plot)
        X_parameters=axis['x']
        Y_parameters=axis['y']

        regr = linear_model.LinearRegression()
        regr.fit(X_parameters, Y_parameters)   #train model
        predict_values=np.arange(0,len(fenxing_plot)-1,1)
        predict_values=np.array(predict_values).reshape(len(fenxing_plot)-1,1)
        ...
        self.zhibiao_ax.plot(predictions["predicted_value"],'r', lw=1)
        return predictions

    def get_axis_high_line(self,fenxing_plot):
        high_line=[]
        length=len(fenxing_plot)
        for i in range(length):
            ...
            if(left<0 or right>length-1):
                ...
                left_value =fenxing_plot[left]
                right_value = fenxing_plot[right]
                value=fenxing_plot[i]

                top_point=value>left_value and value>right_value
                if(top_point):
                    high_line.append(i)
        ...
        axis_high_line=[]
        value_list=[]
        length=len(high_line)
        for i in range(length):
            left = i-1
            if(left<0 ):
                continue
            else:
                left_position=high_line[left]
                position=high_line[i]

                left_value =fenxing_plot[left_position]
                value=fenxing_plot[position]

                top_line_flag= value<left_value
                if(top_line_flag):
                    axis_high_line.append(left_position)
                    axis_high_line.append(position)
                    value_list.append(self.as_num(left_value))
                    value_list.append(self.as_num(value))

        axis_high_line=np.array(axis_high_line).reshape(len(axis_high_line),1)
        axis["x"]=axis_high_line
        axis["y"]=value_list
        return axis
